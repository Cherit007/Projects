...
const AuthScreen = ({ onLogin, onRegister, onContinueAsViewer, loading }) => {
  ...
  return (
    <div className="theme-page min-h-screen flex items-center justify-center p-4">
      <div className="theme-card w-full max-w-md rounded-3xl p-6 sm:p-8">
        ...
        <p className="text-sm text-slate-600 mt-2">Sign in to manage groups or continue as viewer.</p>

        <div className="mt-4 flex gap-2">
          <button
            onClick={() => setMode('login')}
            className={`flex-1 py-2 rounded-xl border ${mode === 'login' ? 'btn-brand border-transparent' : 'bg-white/80 text-slate-700 border-slate-300'}`}
          >
            Login
          </button>
          <button
            onClick={() => setMode('register')}
            className={`flex-1 py-2 rounded-xl border ${mode === 'register' ? 'btn-brand border-transparent' : 'bg-white/80 text-slate-700 border-slate-300'}`}
          >
            Register
          </button>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="mt-3 w-full py-2.5 border border-slate-300 text-slate-700 rounded-xl font-semibold bg-white/80 disabled:opacity-50"
        >
          Continue as Viewer
        </button>
      </div>
