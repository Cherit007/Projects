      ...
      <div className="max-w-5xl mx-auto grid gap-4 sm:gap-5">
        <section className="theme-card rounded-3xl p-6 sm:p-8">
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              onClick={onBack}
              className="px-4 py-2 rounded-xl border border-slate-300 text-slate-700 bg-white/70 hover:bg-white"
            >
              Back
            </button>
          </div>
          ...
          {pendingRequests.length === 0 ? (
            ...
            <div className="mt-3 grid gap-2">
              {pendingRequests.map((request) => (
                <div key={request.id} className="flex flex-wrap items-center justify-between gap-3 border border-slate-200 rounded-lg px-3 py-2 bg-white/70">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      disabled={loading}
                      className="px-3 py-1.5 rounded-md border border-slate-300 text-xs bg-white hover:bg-slate-50 disabled:opacity-50"
                    >
                      Reject
                    </button>
                  </div>
          ...
          {recentReviews.length === 0 ? (
            ...
            <div className="mt-3 grid gap-2">
              {recentReviews.map((item) => (
                <div key={item.id} className="flex items-center justify-between gap-3 border border-slate-200 rounded-lg px-3 py-2 bg-white/70">
                  <div className="min-w-0">
