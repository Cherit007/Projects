...
const buildIdleScenarioLine = ({
  ...
  if (rank > 0 && rank <= 2) {
    return `${team.name}: currently ${rankCopy} with ${pointsCopy}, not qualified yet; a win adds 2 league pts.`;
  }
  ...
  if (futureMatchesForTeam > 1) {
    return `${team.name}: ${rankCopy} with ${pointsCopy}; a win adds 2 league pts, but Top 2 needs later results too.`;
  }
