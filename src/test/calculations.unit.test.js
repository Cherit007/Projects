...
import { generateIplPlayoffBracket } from "../utils/iplPlayoffs";
...
describe("generateKnockoutBracket", () => {
  ...
  it("creates an IPL playoff bracket and advances winners/losers for 4 teams", () => {
    const teams = createTeams(4);
    ...
    expect(next[2][0].team1?.id).toBe(bracket[0][0].team1.id);
    expect(next[1][0].team1?.id).toBe(bracket[0][0].team2.id);

    next = updateBracket(next, 2, 21, 8);
    expect(next[1][0].team2?.id).toBe(bracket[0][1].team1.id);

    next = updateBracket(next, 3, 21, 15);
    expect(next[2][0].team2?.id).toBe(next[1][0].team1.id);

    next = updateBracket(next, 4, 21, 18);
