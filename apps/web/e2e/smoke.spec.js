...
import { TOURNAMENT_NAME_PLACEHOLDER } from '../src/components/setup/sportSetupConfig.js';
...
const openTournamentSetup = async (
  ...
  if (sportLabel) {
    await page.getByRole('radio', { name: sportLabel }).click();
  }
  ...
  await startLane.getByPlaceholder(TOURNAMENT_NAME_PLACEHOLDER).fill(tournamentName);
...
test('smoke @scoring: live scoring flow', async ({ page }) => {
  await loginAsAdmin(page);
  await createLeagueTournament(page);
...
test('smoke @offline: offline write queues sync and shows pending-sync indicator', async ({ page }) => {
  await loginAsAdmin(page);
  await createLeagueTournament(page);
...
test('smoke @auth: join request lifecycle from request to admin approval and member access', async ({ page }) => {
  const requester = {
    name: 'Requester User',
...
test('smoke @scoring @pickleball: pickleball league can start and score a match', async ({ page }) => {
  await loginAsAdmin(page);
  const teams = [
    { teamName: 'Aces', players: ['Amy', 'Alex'] },
    { teamName: 'Dinks', players: ['Dan', 'Dana'] },
    { teamName: 'Smash', players: ['Sam', 'Sky'] },
  ];
  await createTournament(page, {
    tournamentName: 'Pickleball E2E Cup',
    teams,
    sportLabel: 'Pickleball',
  });

  await scoreLiveMatch(page, 11, 7);
  await expect(page.getByText(/Completed Matches \(1\/\d+\)/)).toBeVisible();
});
