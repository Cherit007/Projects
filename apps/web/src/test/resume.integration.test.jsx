...
const unmountApp = async (renderResult) => {
...
const tearDownLiveApp = async (renderResult) => {
  await unmountApp(renderResult);
  flushQueuedLocalStorageWrites();
  resetLocalStorageWriteQueue();
  localStorage.clear();
...
describe('Resume tournament integration', () => {
  ...
  it('after finishing match 1 and refreshing, app resumes at match 2 (not match 1)', async () => {
    ...
    const firstRender = renderApp({ hash: '' });
    ...
    await waitFor(() => {
      ...
      expect(history[0]?.fixtures?.[0]?.completed).toBe(true);
    });

    await unmountApp(firstRender);

    const secondRender = renderApp({ hash: '' });
    expect(await screen.findByText(/Match 2/i, {}, { timeout: ASYNC_UI_TIMEOUT })).toBeInTheDocument();
    await tearDownLiveApp(secondRender);
  }, 15000);
