...
import { getSportScoringConfig } from '@fixture-maker/domain/sports';
import { resolvePickleballScoringRules } from '@fixture-maker/domain/sports/pickleball/pickleballScoring';
import {
...
const PICKLEBALL_TRACKED_STATS = [
  { key: 'aces', label: 'Aces' },
  { key: 'serviceWins', label: 'Service wins' },
];

const resolveQuickScoreRules = (sportId, ruleConfig) => {
  if (sportId === 'pickleball') {
    const { pointCap, winBy, winByRule } = resolvePickleballScoringRules(ruleConfig);
    return { pointCap, winBy, winByRule, deuceThreshold: pointCap - 1 };
  }
  const pointCap = getSportScoringConfig(sportId).quickScores?.[0] ?? 21;
  return { pointCap, winBy: 2, winByRule: 'winBy', deuceThreshold: pointCap - 1 };
};
...
const LiveMatchView = ({ 
  ...
  const [showPickleballStats, setShowPickleballStats] = useState(false);
  const [pickleballStats, setPickleballStats] = useState(() => normalizePickleballMatchStatistics());
  ...
  const quickScoreRules = useMemo(
    () => resolveQuickScoreRules(sportId, ruleConfig),
    [sportId, ruleConfig],
  );
  const quickScorePresets = useMemo(
    () => getSportScoringConfig(sportId).quickScores || [],
    [sportId],
  );
  const isPickleball = sportId === 'pickleball';

  useEffect(() => {
    setPickleballStats(normalizePickleballMatchStatistics());
    setShowPickleballStats(false);
  }, [currentMatch?.id]);

  const getQuickWinnerScores = (winnerTeam) => {
    const { pointCap, winBy, winByRule, deuceThreshold } = quickScoreRules;
    const current1 = Number.parseInt(score1, 10);
    const current2 = Number.parseInt(score2, 10);
    const resolveWinnerScore = (loserScore, currentWinner) => {
      if (winByRule === 'fixedCap') {
        return pointCap;
      }
      const safeLoser = Number.isFinite(loserScore) ? Math.max(0, loserScore) : 0;
      const minWinner = safeLoser >= deuceThreshold ? safeLoser + winBy : Math.max(pointCap, safeLoser + 1);
      const currentWinnerScore = Number.isFinite(currentWinner) ? currentWinner : 0;
      return Math.max(minWinner, currentWinnerScore);
    };
    if (winnerTeam === 1) {
      const loserScore = Number.isFinite(current2) ? Math.max(0, current2) : 0;
      const winnerScore = resolveWinnerScore(loserScore, current1);
    ...
    const winnerScore = resolveWinnerScore(loserScore, current2);
  ...
  const applyQuickScorePreset = (pointCap) => {
    const winBy = quickScoreRules.winBy || 2;
    setScore1(String(pointCap));
    setScore2(String(Math.max(0, pointCap - winBy)));
    hapticTap();
  };

  const adjustPickleballStat = (teamKey, statKey, delta) => {
    setPickleballStats((prev) => {
      const nextValue = Math.max(0, (prev[teamKey]?.[statKey] || 0) + delta);
      return {
        ...prev,
        [teamKey]: {
          ...prev[teamKey],
  ...
  const handleSubmit = async () => {
    ...
    try {
      const hasPickleballStats = isPickleball && PICKLEBALL_TRACKED_STATS.some(
        (entry) => (pickleballStats.team1?.[entry.key] || 0) > 0
          || (pickleballStats.team2?.[entry.key] || 0) > 0,
      );
      const saveExtras = hasPickleballStats ? { statistics: pickleballStats } : undefined;
      const result = await Promise.resolve(
        onSaveScore(currentMatch.id, parsedScore1, parsedScore2, saveExtras),
      );
      if (result === false) {
    ...
    <div className="space-y-3 sm:space-y-4 app-screen-live">
      <div className={`variant-a-card variant-a-live-shell submit-feedback-${submitFeedbackState}`}>
        ...
        {quickScorePresets.length > 0 && (
          <div className="variant-a-score-presets" data-no-gesture="true">
            <p className="variant-a-score-presets-label">Quick scores</p>
            <div className="variant-a-score-presets-row">
              {quickScorePresets.map((pointCap) => (
                <button
                  key={`preset-${pointCap}`}
                  type="button"
                  disabled={isSubmitting}
                  onClick={() => applyQuickScorePreset(pointCap)}
                  className="variant-a-score-preset-btn"
                >
                  {pointCap}-{Math.max(0, pointCap - (quickScoreRules.winBy || 2))}
                </button>
              ))}
            </div>
          </div>
        ...
        {isPickleball && (
          <div className="variant-a-pickleball-stats" data-no-gesture="true">
            <button
              type="button"
              className="variant-a-pickleball-stats-toggle"
              onClick={() => setShowPickleballStats((open) => !open)}
              disabled={isSubmitting}
            >
              {showPickleballStats ? 'Hide optional stats' : 'Track optional stats'}
            </button>
            {showPickleballStats && (
              <div className="variant-a-pickleball-stats-grid">
                {(['team1', 'team2']).map((teamKey, index) => (
                  <div key={teamKey} className="variant-a-pickleball-stats-team">
                    <p className="variant-a-pickleball-stats-team-name">
                      {index === 0 ? currentMatch.team1.name : currentMatch.team2.name}
                    </p>
                    {PICKLEBALL_TRACKED_STATS.map(({ key, label }) => (
                      <div key={`${teamKey}-${key}`} className="variant-a-pickleball-stat-row">
                        <span>{label}</span>
                        <div className="variant-a-pickleball-stat-controls">
                          <button
                            type="button"
                            className="variant-a-pickleball-stat-btn"
                            disabled={isSubmitting}
                            onClick={() => adjustPickleballStat(teamKey, key, -1)}
                            aria-label={`Decrease ${label}`}
                          >
                            <Minus size={14} />
                          </button>
                          <span>{pickleballStats[teamKey]?.[key] || 0}</span>
                          <button
                            type="button"
                            className="variant-a-pickleball-stat-btn"
                            disabled={isSubmitting}
