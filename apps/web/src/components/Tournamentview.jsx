...
import ReportsTab from './tournamentTabs/ReportsTab';
...
const TournamentView = ({
  ...
  const sportMeta = useMemo(() => getSport(sportId), [sportId]);
  const reportTypeIds = useMemo(
    () => getSportPlugin(sportId).reports.listReportTypes(),
    [sportId],
  );
  const showReportsTab = tournamentFormat === 'league' && reportTypeIds.length > 0;
  const {
    activeTab,
    setActiveTab,
    showHeaderMenu,
  ...
  const mobileCommandItems = useMemo(() => {
    ...
    return items;
  }, [activeTab, onGoHome, setActiveTab, showReportsTab, tournamentFormat]);
  ...
  const handleSaveMatchResult = async (matchId, score1, score2, extras) => {
    ...
    try {
      const result = await Promise.resolve(onSaveMatchResult(matchId, score1, score2, extras));
      if (result === false) {
  ...
  const desktopTabItems = [
    ...
    ...(tournamentFormat === 'league'
      ? [
          ...
          ...(showReportsTab ? [{ key: 'reports', label: 'Reports' }] : []),
        ]
      : []),
    { key: 'elo', label: 'ELO' },
              ...
              {isEditingName ? (
                ...
                <div className="flex items-center gap-2 min-w-0">
                  <div className="min-w-0">
                    <h1 className="variant-a-header-title truncate">{sportMeta.icon} {tournamentName}</h1>
