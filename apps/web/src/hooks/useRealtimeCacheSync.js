...
export const useRealtimeCacheSync = ({
  ...
  useEffect(() => {
    ...
    const templatesCollectionId = toText(COLLECTIONS.TOURNAMENT_TEMPLATES);
    const rosterCollectionId = toText(COLLECTIONS.GROUP_ROSTER);
    ...
    if (templatesCollectionId) {
      channels.push(`databases.${DATABASE_ID}.collections.${templatesCollectionId}.documents`);
    }
    if (rosterCollectionId) {
      channels.push(`databases.${DATABASE_ID}.collections.${rosterCollectionId}.documents`);
    }
    if (metaCollectionId) {
    ...
    try {
      unsubscribe = client.subscribe(channels, (message) => {
        ...
        const isAuxiliaryEvent = hasCollectionMarker(markers, playersCollectionId)
          ...
          || hasCollectionMarker(markers, templatesCollectionId)
          || hasCollectionMarker(markers, rosterCollectionId)
